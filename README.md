...
```bash
multi-modal-rag-notes-api$ sam build --use-container
```
...
```bash
multi-modal-rag-notes-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
multi-modal-rag-notes-api$ sam local start-api
multi-modal-rag-notes-api$ curl http://localhost:3000/
```
...
```bash
multi-modal-rag-notes-api$ sam logs -n HelloWorldFunction --stack-name multi-modal-rag-notes-api --tail
```
...
```bash
multi-modal-rag-notes-api$ pip install -r tests/requirements.txt --user
# unit test
multi-modal-rag-notes-api$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
multi-modal-rag-notes-api$ AWS_SAM_STACK_NAME="multi-modal-rag-notes-api" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "multi-modal-rag-notes-api"
```
